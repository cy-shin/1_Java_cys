...
public class ServerService {
	 ...
	 * - 항상 서버가 먼저 실행되어 있는 상태에서 클라이언트가 연결해야 함
	 * 
	 * - 데이터 전송 순서가 보장되고, 수신 여부를 판단하고 오류, 손실 등이 발생하면 재전송
	 * 
	 * Java.net 패키지에서 제공하는 
	 * 
	 ...
	 * 
	 * * Socket
	 * - 프로세스의 양 끝단(프로세스 간의 통신을 담당함)
	 * - Input / Output Stream을 보유하고 있음!
	 * 
	 * * ServerSocket
	 * - 포트와 연결되어 외부 요청을 기다리는 객체
	 * - 클라이언트의 연결 요청이 들어오면 클라이언트의 소켓을 얻어 와 연결하게 된다
	 * 
	...
	public void serverStart() {
		...
		// 원활하고 빠른 통신을 위해서 보조 스트림 사용 
		BufferedReader br = null; // 버퍼로 읽어오기
		...
		try {
			...
			// 바이트 기반인 InputStream is를 연결할 수 있도록
			// InputStreamReader를 사용함 (문자 기반으로 확장)
			...
			// Date now 변수를 sdf 변수가 참조하는 format으로 바꾸어서 str에 저장
			
			pw.println(str); // 서버 -> 클라이언트로 출력
			pw.flush(); // .flush() : 스트림의 내용을 밀어냄 .close() : 스트림의 내용을 밀고 스트림을 종료함
