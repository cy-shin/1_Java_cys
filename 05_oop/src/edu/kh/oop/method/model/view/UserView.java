...
public class UserView {
	...
	// User 참조변수 user를 선언, 현재는 참조하는 객체가 없음(null)
	...
	// 회원 가입 정보를 입력 받을 메서드 : 여기에 적는 이유 : switch문에 적으면 복잡해보여서
	public void signUp() {
		...
		// 회원 가입 서비스를 호출 -> UserService
		user = service.signUp(id, pw, name, age, gender); // User 객체의 주소 반환
		// -> 필드에 존재하는 user 참조 변수에 반환된 User 객체의 주소를 저장한다.
	...
	public void login() {
		...
		if(loginUser != null) {
			...
			int result = service.login(id, pw, user); // 로그인 서비스를 호출
			// -1, 0, 1
